...
void App_Update_Send_Update_Cmd(void)//使用lora发送更新指令
{
...
void App_Update_Receive_App_Data(void)//CAN接受程序数据
{
    ...
    if(res==0)
    {
        ...
        memcpy(app_update_buff+lora_rec_msg_len , gs_handle.receive_buf , gs_handle.receive_buf_len);
...
void App_Update_Check_Data(void)//检查app_update_buff中的数据
{
    ...
    if(res==0)
    {
        uint32_t crc_val_F1=gs_handle.receive_buf[0] | (gs_handle.receive_buf[1]<<8) | (gs_handle.receive_buf[2]<<16) | (gs_handle.receive_buf[3]<<24);
        uint32_t crc_val_F4=App_Crc_Cal(app_update_buff, lora_rec_msg_len);
...
void App_Update_Change_Boot_Mode(void)//修改在AT24C02中的更新标志位，将程序保存到W25Q64中
{
    ...
    w25q64_write_buff[4]=lora_rec_msg_len & 0xff;
    w25q64_write_buff[5]=(lora_rec_msg_len>>8)&0xff;
    w25q64_write_buff[6]=(lora_rec_msg_len>>16)&0xff;
    w25q64_write_buff[7]=(lora_rec_msg_len>>24)&0xff;
    int_W25Q64_Write_Data_With_32Addr(W25Q64_FLASH_META_ADDR,w25q64_write_buff,8);
